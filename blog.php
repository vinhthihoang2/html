<main>
    <div class="container-fluid blog_main_banner_img px-0 d-flex">
        <div class="col-3"><img class="img-fluid about_us_banner_bg_left" src="img/about_us/banner_bg_left.png"></div>

        <div class="container col-6 d-flex flex-column align-items-center justify-content-center text-center">
            <div class="about_us_banner_title pb-5">Blog</div>
            <div class="about_us_banner_context col-10">
                Lorem ipsum dolor sit amet, consectetur adipiscing elit. Arcu condimentum mattis vitae sem convallis sit cras pellentesque facilisis. Morbi commodo.
            </div>
        </div>

        <div class="col-3"><img class="img-fluid about_us_banner_bg_right" src="img/about_us/banner_bg_right.png"></div>
    </div>

    <div class="container-fluid blog_main_content" style="padding: 150px 0">
        <div class="container">
            <div class="content_title text-center" style="font-weight: 700; padding-bottom: 60px">Today’s Blog</div>

            <div class="row">
                <div class="col-lg-6 col-12" style="padding-bottom: 150px">
                    <div class="text-center">
                        <img class="img-fluid w-100" src="img/blog/content_img_1.png">
                        <div class="d-flex" style="padding: 50px 0 22px 0;">
                            <div class="blog_date text-start">Alex S - May 13, 2022</div>
                            <img class="blog_like_btn ms-auto" src="img/blog/like_heart.png">
                        </div>
                        <div class="text-start blog_context col-10">Unforgettable Memories Are Vacations</div>
                    </div>

                </div>

                <div class="col-lg-6 col-12" style="padding-bottom: 150px">
                    <div class="text-center">
                        <img class="img-fluid w-100" src="img/blog/content_img_2.png">
                        <div class="d-flex" style="padding: 50px 0 22px 0;">
                            <div class="blog_date text-start">Alex S - May 13, 2022</div>
                            <img class="blog_like_btn ms-auto" src="img/blog/like_heart.png">
                        </div>
                        <div class="text-start blog_context col-10">Mountain Tourist Places Suitable for Couples</div>
                    </div>

                </div>

                <div class="col-lg-6 col-12" style="padding-bottom: 150px">
                    <div class="text-center">
                        <img class="img-fluid w-100" src="img/blog/content_img_3.png">
                        <div class="d-flex" style="padding: 50px 0 22px 0;">
                            <div class="blog_date text-start">Alex S - May 13, 2022</div>
                            <img class="blog_like_btn ms-auto" src="img/blog/like_heart.png">
                        </div>
                        <div class="text-start blog_context col-10">Capture Your Vacation Moments For Memories</div>
                    </div>

                </div>

                <div class="col-lg-6 col-12" style="padding-bottom: 150px">
                    <div class="text-center">
                        <img class="img-fluid w-100" src="img/blog/content_img_4.png">
                        <div class="d-flex" style="padding: 50px 0 22px 0;">
                            <div class="blog_date text-start">Alex S - May 13, 2022</div>
                            <img class="blog_like_btn ms-auto" src="img/blog/like_heart.png">
                        </div>
                        <div class="text-start blog_context col-10">Mountain Tourist Places Suitable for Couples</div>
                    </div>

                </div>
            </div>

            <div class="d-flex justify-content-center">
                <ul class="pagination border-0 align-items-center">
                    <li class="page-item me-5">
                        <a class="page-link border-0"><img src="img/blog/Stroke-1.png"></a>
                    </li>
                    <li class="page-item px-1">
                        <a id="page_number" class="page-link border-0">1</a>
                    </li>
                    <li class="page-item px-1">
                        <a id="page_number" class="page-link border-0">2</a>
                    </li>
                    <li class="page-item px-1">
                        <a id="page_number" class="page-link border-0">3</a>
                    </li>
                    <li class="page-item px-1">
                        <a id="page_number" class="page-link border-0">4</a>
                    </li>
                    <li class="page-item px-1">
                        <a id="page_number" class="page-link border-0">5</a>
                    </li>
                    <li class="page-item px-1">
                        <a id="page_number" class="page-link border-0">...12</a>
                    </li>
                    <li class="page-item ms-5">
                        <a class="page-link border-0"><img src="img/blog/Stroke-2.png"></a>
                    </li>
                </ul>
            </div>
        </div>
    </div>

    <div class="container-fluid blog_off_today d-flex align-items-center">
        <div class="container">
            <div class="" >
                <div class="off_today_title_1">25%<span class="off_today_title_2"> OFF Today’s!!</span></div>
                <div class="off_today_context py-5 col-7">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Arcu condimentum mattis vitae sem convallis sit cras
                </div>
                <button class="btn_explore_1 py-3 px-4 my-4 d-inline-block">Contact Us<img class="ps-2" src="img/Home1/arrow_1.png"></button>
                <div class="off_today_media my-3">
                    Our Social Media
                </div>
                <div class="header_icon_nav">
                    <a class="header_icon me-2" href=""><img src="img/Home1/instagram_icon.png"></a>
                    <a class="header_icon me-2" href=""><img src="img/Home1/facebook_icon.png"></a>
                    <a class="header_icon me-2" href=""><img src="img/Home1/youtube_icon.png"></a>
                    <a class="header_icon me-2" href=""><img src="img/Home1/twitter_icon.png"></a>
                </div>
            </div>
        </div>
    </div>

    <div class="container-fluid blog_bg pb-5">
        <div class="container blog">

            <div class="container say_about_title mt-5 text-center py-5">What Say About Us?</div>

            <div id="carouselExampleInterval" class="carousel slide pb-5" data-bs-ride="carousel">
                <div class="carousel-inner">
                    <div class="carousel-item active" data-bs-interval="10000">
                        <div class="col-6 text-center mx-auto say_about_context px-3">“Lorem ipsum dolor sit amet, consectetur adipiscing elit. Arcu condimentum mattis vitae sem convallis sit cras pellentesque”</div>
                        <div class="d-flex justify-content-center py-5">
                            <img src="img/package_offer/wsau_img.png">
                            <div class="align-self-center ps-4">
                                <div class="say_about_name">Anjas Claude</div>
                                <div class="say_about_name_title">Traveller International</div>
                            </div>
                        </div>
                    </div>
                    <div class="carousel-item" data-bs-interval="2000">
                        <div class="col-6 text-center mx-auto say_about_context px-3">“Lorem ipsum dolor sit amet, consectetur adipiscing elit. Arcu condimentum mattis vitae sem convallis sit cras pellentesque”</div>
                        <div class="d-flex justify-content-center py-5">
                            <img src="img/package_offer/wsau_img.png">
                            <div class="align-self-center ps-4">
                                <div class="say_about_name">Anjas Claude</div>
                                <div class="say_about_name_title">Traveller International</div>
                            </div>
                        </div>
                    </div>
                    <div class="carousel-item">
                        <div class="col-6 text-center mx-auto say_about_context px-3">“Lorem ipsum dolor sit amet, consectetur adipiscing elit. Arcu condimentum mattis vitae sem convallis sit cras pellentesque”</div>
                        <div class="d-flex justify-content-center py-5">
                            <img src="img/package_offer/wsau_img.png">
                            <div class="align-self-center ps-4">
                                <div class="say_about_name">Anjas Claude</div>
                                <div class="say_about_name_title">Traveller International</div>
                            </div>
                        </div>
                    </div>
                </div>
                <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleInterval" data-bs-slide="prev">
                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                    <span class="visually-hidden">Previous</span>
                </button>
                <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleInterval" data-bs-slide="next">
                    <span class="carousel-control-next-icon" aria-hidden="true"></span>
                    <span class="visually-hidden">Next</span>
                </button>
            </div>

        </div>
    </div>
</main>